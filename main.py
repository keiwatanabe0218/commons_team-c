...
if __name__ == "__main__":
    ...
    cams[0].factor = [0 for i in range(len(srf_vectors))]
    cams[1].factor = [0 for i in range(len(srf_vectors))]
        ...
        try:
            ...
            for cam in cams:
                ...
                if face is not None:
                    ...
                    xy = face["yaw"]
                    yz = face["pitch"]
                    cam.min_difs, cam.max_difs, cam.factor = vector_to_value(index, srf_vectors, xy, yz, xy_old_min, xy_old_max, xy_new_min, xy_new_max, yz_old_min, yz_old_max, yz_new_min, yz_new_max)
            ...
            if cams[0].none_count > 5 and cams[1].none_count > 5:
                factor_a = [0 for i in range(len(srf_vectors))]
                factor_b = [0 for i in range(len(srf_vectors))]
                # cams[1].none_count = 0
            # cam0のNoneが５回連続したら
